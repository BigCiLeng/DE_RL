...
def test_dqn(arg, norm_data, ori_rank, args=get_args()):
    ...
    if torch.cuda.is_available():
        if os.path.exists(os.path.join(args.logdir, 'doudqn/policy.pth')):
            policy.load_state_dict(torch.load(os.path.join(args.logdir, 'doudqn/policy.pth')))
    else:
        if os.path.exists(os.path.join(args.logdir, 'doudqn/policy.pth')):
            policy.load_state_dict(torch.load(os.path.join(args.logdir, 'doudqn/policy.pth'),map_location='cpu'))
    # collector
    ...
    # log
    log_path = os.path.join(args.logdir, 'doudqn')
