...
def get_args():
    ...
    parser.add_argument(
        '--dueling-v-hidden-sizes', type=int, nargs='*', default=[64, 32]
    )
    ...
    parser.add_argument('--frames_stack',type=int,default=8)
...
def test_dqn(arg,norm_data,ori_rank,args=get_args()):
    ...
    if os.path.exists(os.path.join(args.logdir,'ddqn/policy.pth')):
        policy.load_state_dict(torch.load(os.path.join(args.logdir,'ddqn/policy.pth')))
    # collector
    ...
    # log
    log_path = os.path.join(args.logdir,'ddqn')
